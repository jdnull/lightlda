...
namespace multiverso { namespace lightlda
{
	...
	bool IsModelFile(std::string filename, int& table_type)
	{	// filename: server_0_table_0.model
		if (filename.substr(0, 7) != "server_") return false;

		std::string::size_type st = filename.find_first_of('_', 7); // st: 8
		std::string digit_str = filename.substr(7, st - 7); // digit_str: "0"
		if (!IsDigit(digit_str)) return false;

		if (filename.substr(st, 7) != "_table_") return false;

		st = filename.find_last_of('_'); // st: 14
		digit_str = filename.substr(st + 1, 1); // digit_str: "0"
		if (!IsDigit(digit_str)) return false;
		table_type = atoi(digit_str.c_str());

		if (!(filename.substr(st + 2, 6) == ".model" && st + 8 == filename.size())) return false;

		return true;
	}

	bool IsLoadWordTopicTable(std::string filename)
	{
		int table_type;
		if (IsModelFile(filename, table_type))
		{
			return table_type == multiverso::lightlda::kWordTopicTable ? true : false;
		}
		return false;
	}

	bool IsLoadSummaryTable(std::string filename)
	{
		int table_type;
		if (IsModelFile(filename, table_type))
		{
			return table_type == multiverso::lightlda::kSummaryRow ? true : false;
		}
		return false;
	}
#endif
    ...
    void LocalModel::LoadTable()
    {
		...
		std::string path_name = Config::input_dir + "\\*";
		size_t str_len = path_name.length();
		wchar_t* buf = new wchar_t[str_len + 1];
		MultiByteToWideChar(CP_ACP, 0, path_name.c_str(), str_len, buf, str_len*sizeof(wchar_t));
		buf[str_len] = 0;
		if ((h_file = FindFirstFile(buf, &file_info)) == INVALID_HANDLE_VALUE)
		{
			Log::Fatal("model dir does not exist : %s\n", Config::input_dir.c_str());
		}
		delete buf;
		bool wordtopic_flag = false;
		bool summary_flag = false;
		do
		{
			std::string filename = WChar2Ansi(file_info.cFileName);
			if (IsLoadWordTopicTable(filename))
			{
				wordtopic_flag = true;
				Log::Info("loading word topic table[%s]\n", filename.c_str());
				LoadWordTopicTable(Config::input_dir + "/" + filename);
			}
			else if (IsLoadSummaryTable(filename))
			{
				summary_flag = true;
				Log::Info("loading summary table[%s]\n", filename.c_str());
				LoadSummaryTable(Config::input_dir + "/" + filename);
			}
		} while (FindNextFile(h_file, &file_info) != 0);
		FindClose(h_file);

		if (!(wordtopic_flag && summary_flag))
		{
			Log::Fatal("model files is not complete : %s\n", Config::input_dir.c_str());
		}
